...
# ln EGA_hg19/${name}/splits/* ./splits
...
# ------ 2025.10.18
# 重新整理 EGA 里面所有的软连接
find /cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA \
    -type l -regex ".*EGA/GBM.*\|.*/splits/.*" -print0 \
| xargs -0 -I{} sh -c 'echo -e "{}\t$(readlink "{}")"' > \
 /cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA/EGA_links.txt

# 替换link右侧的/cluster/至/cluster2/
sed -i 's/\/cluster\//\/cluster2\//g' /cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA/EGA_links.txt
sed -i 's/tmp/futing\/Project/g' /cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA/EGA_links.txt
awk 'BEGIN{FS=OFS="\t"}$2 ~ /\.\// {$2 = $1; gsub("splits", "fastq", $2)} 1' /cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA/EGA_links.txt > tmp \
	&& mv tmp /cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA/EGA_links.txt

IFS=$'\t'
while read -r clk sourcelk;do
	if [ -L "$clk" ]; then
		# 当前软链真实指向
		target=$(readlink -f "$clk")
		expected=$(readlink -f "$sourcelk")

		if [ "$target" = "$expected" ]; then
			echo "symlink exists and is correct: $clk → $target"
		else
			echo "symlink exists but points to WRONG target: $target"
			echo "fixing..."
			rm "$clk"
			ln -s "$sourcelk" "$clk"
		fi
...
done < "/cluster2/home/futing/Project/GBM/HiC/02data/01fastq/EGA/EGA_links.txt"
